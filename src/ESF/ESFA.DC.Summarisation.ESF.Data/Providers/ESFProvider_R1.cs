            ...
            using (var esfContext = _esf())
            {
                var preSummarised = await esfContext.SupplementaryDatas
                                ...
                                .Select(obj => new 
                                {
                                    ...
                                    Value = obj.Sum(p => p.CostType.Equals(ConstantKeys.UnitCost, StringComparison.OrdinalIgnoreCase) ? (p.SupplementaryDataUnitCost.Value ?? p.Value) : p.CostType.Equals(ConstantKeys.UnitCostDeduction, StringComparison.OrdinalIgnoreCase) ? (p.SupplementaryDataUnitCost.Value ?? p.Value) : p.Value),
